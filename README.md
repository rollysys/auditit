...
# 场景 2：手动模式 —— auditit 只开审计窗格，打印命令让你自己起
...
| 命令 | 说明 |
|---|---|
| `start [--model MODEL] [--repo-root DIR]` | 打开 3 窗格审计布局；带 `--model` 则同时启动托管 worker |
| `launch --prompt STR [--model MODEL] [--max-turns N]` | 以 `claude -p` headless 模式运行，同时显示审计窗格 |
